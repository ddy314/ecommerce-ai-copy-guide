...
[backend/api/auth_routes.py](file:///c:/Users/Jin/Desktop/ecommerce-ai-copy-guide-main/backend/api/auth_routes.py) 负责登录注册与头像上传。为解决用户上传 `.jpg` 文件仍被拦截的问题，校验逻辑同时参考文件名扩展名与 MIME 类型：
...
上传接口返回相对路径 `/uploads/avatars/xxx.jpg`，由前端统一拼接为完整 URL。
